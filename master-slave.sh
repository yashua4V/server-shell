...
yum -y install net-tools &>/deb/null && echo ifconfig安装成功
...
echo $pass_word > /root/.mysql_newpass
expect <<  EOF
spawn mysql -uroot  -p
...
if [ $server_id -eq 4 ];then
    ...
    mysql -uroot -p$pass_word -e "start slave;" && echo 从库启动中
    sleep 10
